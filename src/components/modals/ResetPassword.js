...
import SiteContextProvider from "../../context/SiteContext/SiteContext";

export default function ResetPassword({ email, phone, ...props }) {
  ...
  const onCheck = async () => {
    try {
      ...
      setisDisabled(true);
      Service.resetPasswordCheck(values)
        .then((res) => {
